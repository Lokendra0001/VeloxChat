...
const AddContact = ({ status, onCloseAddContact }) => {
  ...
  return (
    <div className="max-w-md w-full z-[100] bg-white dark:bg-secondary sm:rounded-xl shadow-md overflow-hidden flex flex-col h-full sm:h-[85dvh] select-none relative">
      ...
      {/* Header */}
      <div className="bg-gradient-to-br bg-primary dark:bg-primary-hover p-4 text-white">
      ...
      <div className="p-4 border-b border-light-border">
        <div className="relative flex items-center">
          <div className="absolute left-3 text-gray-400 dark:text-text-secondary">
          ...
          />
          <button className="ml-1 px-4 py-2 bg-primary hover:bg-primary-hover dark:bg-primary-hover dark:hover:bg-primary text-white rounded-sm font-medium transition-colors shadow-sm">
            Search
          </button>
        </div>
      </div>
      ...
      {/* Contact List */}
      <div className="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-light-border space-y-1">
        {filteredUsers.length > 0 ? (
          filteredUsers.map((user, index) => {
            ...
            return (
              <div
                key={user._id || index}
                className="flex items-center gap-4 p-3 hover:bg-gray-50 dark:hover:bg-secondary/50 transition-all"
              >
                {/* Profile */}
                <div className="relative h-10 w-10">
                  ...
                  {/* Online status */}
                  <div
                    className={`absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-white dark:border-secondary shadow ${
                      status === "online"
                        ? "bg-green-500"
                        : status === "recent"
                        ? "bg-yellow-500"
                ...
                <div className="flex-1">
                  <h2 className="text-gray-800 dark:text-text-primary font-semibold capitalize">
                    {user.username}
                  </h2>
                  {status !== "offline" && (
                    <p
                ...
                <button
                  ...
                  className={`text-xs px-2 py-1.5 rounded shadow-md transition-all duration-300 flex items-center gap-1 ${
                    isSent
                      ? isAccepted
                        ? "bg-gray-100 dark:bg-user-bubble dark:text-other-bubble  cursor-not-allowed"
                        : "bg-other-bubble dark:bg-secondary text-gray-700 dark:text-text-primary cursor-not-allowed"
                      : "bg-primary hover:bg-primary-hover dark:bg-primary-hover dark:hover:bg-primary text-white cursor-pointer"
                  }`}
                  disabled={isSent}
                >
                  {isSent ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-10">
            <div className="w-20 h-20 bg-gray-100 dark:bg-secondary rounded-full flex items-center justify-center mb-2">
              <User className="w-10 h-10 text-gray-400 dark:text-text-secondary" />
            </div>
      ...
      {/* Footer */}
      <div className="bg-gray-50 dark:bg-secondary p-3 text-center border-t border-gray-200 dark:border-light-border">
